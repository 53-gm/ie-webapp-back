...
class Lecture(models.Model):
    ...
    departments = models.ManyToManyField(
        "accounts.Department", related_name="lectures", verbose_name="履修可能学科"
    )
    terms = models.ManyToManyField(
        "Term", related_name="lectures", verbose_name="ターム"
    )
...
class Schedule(models.Model):
    ...
    def __str__(self):
        return f"{dict(self.DAY_CHOICES).get(self.day)} {dict(self.TIME_CHOICES).get(self.time)}"
...
class Term(models.Model):
    ...
    def clean(self):
        if self.number not in dict(self.TERM_CHOICES):
...
class Task(models.Model):
    ...
    priority = models.SmallIntegerField(verbose_name="優先度", default=0)
    status = models.SmallIntegerField(
        verbose_name="状態", default=0
    )  # 0 = 未着手, 1 = 進行中, 2 = 完了
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="登録日時")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新日時")
